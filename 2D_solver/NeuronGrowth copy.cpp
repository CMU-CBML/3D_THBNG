...
NeuronGrowth::NeuronGrowth(){
	...
	// png_save_invl 		= 100; 		// png_save_invl
	// png_plot_invl 		= 100; 		// png_plot_invl
	gc_sz			= 2;	     	// gc_sz
	// M_axon			= 100;		// M_axon
...
void NeuronGrowth::InitializeProblemNG(const int n_bz, vector<Vertex2D>& cpts, vector<Vertex2D> prev_cpts, vector<vector<float>> &NGvars, vector<array<int, 2>> &seed)
{
	...
	ierr = MatMPIAIJSetPreallocation(J, 40, NULL, 40, NULL);
	...
	ierr = MatMPIAIJSetPreallocation(GK_syn, 40, NULL, 40, NULL);
	...
	ierr = MatMPIAIJSetPreallocation(GK_tub, 40, NULL, 40, NULL);
...
// 					if (SearchPair(prev_cpts, floorf(x), round(y), indDown) &&
// 						SearchPair(prev_cpts, floorf(x)+1, round(y), indUp)) {
// 						phi[i] = (NGvars[0][indDown] + NGvars[0][indUp])/2;
// 						syn[i] = (NGvars[1][indDown] + NGvars[1][indUp])/2;
// 						tub[i] = (NGvars[2][indDown] + NGvars[2][indUp])/2;
// 						theta[i] = (NGvars[3][indDown] + NGvars[3][indUp])/2;
// 						phi_0[i] = (NGvars[4][indDown] + NGvars[4][indUp])/2;
// 						tub_0[i] = (NGvars[5][indDown] + NGvars[5][indUp])/2;
// 					} else {
...
// 					}
// 				} else if ((abs(remainder(x,1)) != 0.5) && (abs(remainder(y,1)) == 0.5)) {
// 					if (SearchPair(prev_cpts, round(x), floorf(y), indDown) &&
// 						SearchPair(prev_cpts, round(x), floorf(y)+1, indUp)) {
// 						phi[i] = (NGvars[0][indDown] + NGvars[0][indUp])/2;
// 						syn[i] = (NGvars[1][indDown] + NGvars[1][indUp])/2;
// 						tub[i] = (NGvars[2][indDown] + NGvars[2][indUp])/2;
...
// 					}
// 				} else if ((abs(remainder(x,1)) == 0.5) && (abs(remainder(y,1)) == 0.5)) {
// 					if (SearchPair(prev_cpts, floorf(x), floorf(y), indDown) &&
// 						SearchPair(prev_cpts, floorf(x)+1, floorf(y), indUp) &&
// 						SearchPair(prev_cpts, floorf(x), floorf(y), indLeft) &&
// 						SearchPair(prev_cpts, floorf(x), floorf(y)+1, indRight)) {
// 						phi[i] = (NGvars[0][indDown] + NGvars[0][indUp] + NGvars[0][indLeft] + NGvars[0][indRight])/4;
// 						syn[i] = (NGvars[1][indDown] + NGvars[1][indUp] + NGvars[1][indLeft] + NGvars[1][indRight])/4;
// 						tub[i] = (NGvars[2][indDown] + NGvars[2][indUp] + NGvars[2][indLeft] + NGvars[2][indRight])/4;
// 						theta[i] = (NGvars[3][indDown] + NGvars[3][indUp] + NGvars[3][indLeft] + NGvars[3][indRight])/4;
// 						phi_0[i] = (NGvars[4][indDown] + NGvars[4][indUp] + NGvars[4][indLeft] + NGvars[4][indRight])/4;
// 						tub_0[i] = (NGvars[5][indDown] + NGvars[5][indUp] + NGvars[5][indLeft] + NGvars[5][indRight])/4;
// 					} else {
...
// 								// NG.PopulateRandom(NGvars[3]); // replace 0 with random number (for theta)
...
// 	// // Allocation in advance to save computational cost
// 	// nen = 16;
// 	// EVectorSolve.reserve(nen*sizeof(float));
// 	// EMatrixSolve.resize(nen);
// 	// EVectorSolve.resize(nen);

// 	// eleVal.reserve(8*sizeof(float));
// 	// eleVal.resize(8);
...
// 	// Nx.reserve(16*sizeof(float));
// 	// dNdx.reserve(16*2*sizeof(float));
...
void NeuronGrowth::ElementEvaluationAll_phi(const int nen, const vector<float> &Nx, vector<array<float, 2>> &dNdx, const vector<float> elePhiGuess, float &elePG, const vector<float> elePhi, float &eleP, const vector<float> eleSyn, float &eleS, const vector<float> eleTips, float &eleTp, const vector<float> eleTubulin, float &eleTb, float &dPGdx, float &dPGdy)
{
	elePG = 0.;
	eleP = 0.;
	eleS = 0.;
...
void NeuronGrowth::ElementEvaluationAll_phi(const int nen, const vector<float> &Nx, vector<array<float, 2>> &dNdx, vector<vector<float>> &eleVal, vector<float> &vars)
{
	...
	for (int i = 0; i < nen; i++) {
		...
		vars[3] += eleVal[0][i] * dNdx[i][0];	vars[4] += eleVal[0][i] * dNdx[i][1];
		vars[13] += eleVal[6][i] * dNdx[i][0];	vars[14] += eleVal[6][i] * dNdx[i][1];
		vars[16] += eleVal[7][i] * dNdx[i][0];	vars[17] += eleVal[7][i] * dNdx[i][1];
	}
}
...
void NeuronGrowth::prepareEpsilon() {

	/*Build linear system in each process*/
	vector<float> Nx;
	vector<array<float, 2>> dNdx;

	// int nen = bzmesh_process[0].IEN.size();
	int nen = 16;

	vector<float> elePhi, eleTheta, eleEpsilon, eleEpsilonP;
	elePhi.resize(nen);
	eleTheta.resize(nen);
	eleEpsilon.resize(nen);
	eleEpsilonP.resize(nen);
	float eleEP, eleEEP, dAdx, dAdy, dAPdx, dAPdy;
	...
	for (int e = 0; e < bzmesh_process.size(); e++) {
		...
		for (int i = 0; i < Gpt.size(); i++) {
			for (int j = 0; j < Gpt.size(); j++) {
				...
				EvaluateOrientation(nen, Nx, dNdx, elePhi, eleTheta, eleEpsilon, eleEpsilonP);
				...
				pre_dAPdx.push_back(dAPdx);
				pre_dAPdy.push_back(dAPdy);
			}
		}
	}
}
...
// void NeuronGrowth::BuildLinearSystemProcessNG_phiNR(const vector<Element2D> &tmesh, const vector<Vertex2D> &cpts)
// {	
// 	/*Build linear system in each process*/
// 	int e;
// 	for (e = 0; e < bzmesh_process.size(); e++) {
...
// 		vector<float> elePhiGuess, elePhi, eleSyn, eleTubulin, eleTheta, eleTips, eleEnergy, eleEpsilon, eleEpsilonP;
// 		elePhiGuess.resize(nen);
// 		elePhi.resize(nen);
// 		eleSyn.resize(nen);
// 		eleTubulin.resize(nen);
// 		eleTheta.resize(nen);
// 		eleTips.resize(nen);
// 		eleEnergy.resize(nen);
// 		eleEpsilon.resize(nen);
// 		eleEpsilonP.resize(nen);
// 		float C1, elePG, dPGdx, dPGdy, eleP, eleS, eleTb, eleE, dThedx, dThedy, eleEP, dAdx, dAdy, eleEEP, dAPdx, dAPdy;

// 		for (int i = 0; i < nen; i++) {
// 			elePhiGuess[i] = phi_guess[bzmesh_process[e].IEN[i]];
// 			elePhi[i] = phi[bzmesh_process[e].IEN[i]];
// 			eleSyn[i] = syn[bzmesh_process[e].IEN[i]];
// 			eleTubulin[i] = tub[bzmesh_process[e].IEN[i]];
// 			eleTheta[i] = theta[bzmesh_process[e].IEN[i]];
// 			eleTips[i] = tips[bzmesh_process[e].IEN[i]];
// 			// eleEnergy[i] = alphaOverPi*atan(gamma*(1-eleSyn[i]));
// 			eleEpsilon[i] = 0;
// 			eleEpsilonP[i] = 0;
// 		}

// 		float terma2, termadx, termady, termdbl;
// 		float eleTp, term_change;
...
// 				ElementValue(Nx, elePhiGuess, elePG);
// 				ElementValue(Nx, elePhi, eleP);
// 				ElementValue(Nx, eleSyn, eleS);
// 				ElementValue(Nx, eleTips, eleTp);
// 				ElementValue(Nx, eleTubulin, eleTb);
// 				// ElementValue(Nx, eleEnergy, eleE);
// 				ElementValue(Nx, eleEpsilon, eleEP);
// 				ElementValue(Nx, eleEpsilonP, eleEEP);

// 				ElementDeriv(nen, dNdx, elePhiGuess, dPGdx, dPGdy);
// 				ElementDeriv(nen, dNdx, eleTheta, dThedx, dThedy);
// 				ElementDeriv(nen, dNdx, eleEpsilon, dAdx, dAdy);
// 				ElementDeriv(nen, dNdx, eleEpsilonP, dAPdx, dAPdy);

// 				if (n < 1) {
// 					eleE = alphaOverPi*atan(gamma*(1-eleS));
// 				} else {
// 					if (eleTp > 0) {
// 						term_change = regular_Heiviside_fun(50 * eleTb - 0);
// 						// PetscPrintf(PETSC_COMM_WORLD, "tc tip %f | eleTb...%f\n", term_change, eleTb);
// 					} else {
// 						term_change = regular_Heiviside_fun(r * eleTb - g);
// 						// PetscPrintf(PETSC_COMM_WORLD, "no tc tip %f | eleTb...%f\n", term_change, eleTb);
// 					}
// 					eleE = alphaOverPi  *atan(gamma * term_change * (1 - eleS));
// 				}
// 				// PetscPrintf(PETSC_COMM_WORLD, "Function eleE...%f\n", eleE);

// 				C1 = eleE - (0.5 + 6 * s_coeff * sqrt(pow(dThedx, 2) + pow(dThedy, 2)));
...
// 			// if (cpts[A].label == 1)
// 			if (cellBoundary(phi[A], 1e-2) == 0)
// 				ApplyBoundaryCondition(0, i, 0, EMatrixSolve, EVectorSolve);
// 		}
	
// 		ResidualAssembly(EVectorSolve, bzmesh_process[e].IEN, GR_phi);
// 		// if (judge_phi == 0) // The matrix is the same, so only need to assembly once
...
// void NeuronGrowth::BuildLinearSystemProcessNG_phi(const vector<Element2D> &tmesh, const vector<Vertex2D> &cpts)
// {	
// 	/*Build linear system in each process*/
// 	int ind(0);
// 	int e;
...
// 		float terma2, termadx, termady, termdbl, C0;
// 		float eleTp, term_change;
...
// 				// toc(t_check);
// 				// std::cout << "Extraction: " << t_check << std::endl;
// 				// tic();
				
// 				EvaluateOrientation(nen, Nx, dNdx, elePhi, eleTheta, eleEpsilon, eleEpsilonP);
// 				ElementEvaluationAll_phi(nen, Nx, dNdx, elePhiGuess, elePG, elePhi, eleP, eleSyn, eleS, eleTips, eleTp,\
// 					eleTubulin, eleTb, eleEpsilon, eleEP, eleEpsilonP, eleEEP, dPGdx, dPGdy, dAdx, dAdy, dAPdx, dAPdy);
// 				// user->ElementEvaluationAll_phi(nen, Nx, dNdx, elePhiGuess, elePG, elePhi, eleP, eleSyn, eleS, eleTips, eleTp,\
// 				// 	eleTubulin, eleTb, dPGdx, dPGdy);
...
// 					termdbl = (- eleP * eleP * eleP + (1 - C1) * eleP * eleP + C1 * eleP) * Nx[m];
// 					EVectorSolve[m] += (dt * M_phi * (terma2 - termadx + termady + termdbl) + eleP * Nx[m]) * detJ;

// 					for (int n = 0; n < nen; n++) {
// 						EMatrixSolve[m][n] += Nx[m] * Nx[n] * detJ;
// 					}
// 				}
// 			}
// 		}
...
void NeuronGrowth::BuildLinearSystemProcessNG_syn_tub(const vector<Element2D> &tmesh, const vector<Vertex2D> &cpts)
{
	...
	for (int e = 0; e < bzmesh_process.size(); e++) {
		...
		for (int i = 0; i < nen * 1; i++) {
			...
			// eleVal_st[0][i] = round(phi[bzmesh_process[e].IEN[i]]) - round(phi_prev[bzmesh_process[e].IEN[i]]);		// elePhiDiff
			// eleVal_st[1][i] = syn[bzmesh_process[e].IEN[i]];						// eleSyn
			// eleVal_st[2][i] = round(phi[bzmesh_process[e].IEN[i]]);						// elePhi
			// eleVal_st[3][i] = round(phi_prev[bzmesh_process[e].IEN[i]]);						// elePhiPrev
			// eleVal_st[4][i] = tub[bzmesh_process[e].IEN[i]];						// eleConct
		}
...
{
	// 0 - left | 1 - top | 2 - right | 3 - bottom | 5 - no action
	for (int i = 0; i < 10; i++) {
		for (int j = 1; j < NY; j++) {
...
float NeuronGrowth::RmOutlier(vector<float> &data) 
{
	...
	for(int i = 0; i < data.size(); i++) {
		...
		// if (cpts[i].label != 1) {
		// 	if (data[i] < (mean + 1*standardDeviation)) {
		// 		cpts[i].label = 3;
		// 	} else {
		// 		cpts[i].label = 2;
		// 	}
...
// void NeuronGrowth::DetectTips(vector<float> phi, vector<float> &tips)
// {
...
// void NeuronGrowth::DetectTips(vector<float> phi, vector<float> &tips, int NX, int NY)
// {
// 	float threshold = 0.90;
...
void NeuronGrowth::DetectTips(vector<float> phi_in, vector<float> id, int numNeuron, vector<float> &tips, int NX, int NY)
{
	float threshold(0.90), maxVal(0);
	int ind, length(phi_in.size());
	tips.clear();
	tips.resize(length);
	...
	for (int i = (5*NY+5); i < (length-4*NY-4); i++) {
		...
		if (CellBoundary(phi_in[i], 0.25) > 0) {
			for (int j = -4; j < 5; j++) {
				for (int k = -4; k < 5; k++) {
			...
			if (tips[i] > 0)
				// tips[i] = CellBoundary(phi[i], 0.1) / tips[i];
				tips[i] = CellBoundary(phi_in[i], 0) / tips[i];
			if (std::isnan(tips[i]))
				tips[i] = 0;
...
// Function to perform flood fill
void NeuronGrowth::IdentifyNeurons(vector<vector<int>>& neurons, vector<array<int, 2>> seed, int NX, int NY, int originX, int originY) 
{
	...
	// if (comRank == 0) {
	// 	// Output the updated image after flood fill
...
// Function to calculate geodesic distance from a point
vector<vector<int>> NeuronGrowth::CalculateGeodesicDistanceFromPoint(vector<vector<int>> neurons, vector<array<int, 2>> &seed, int originX, int originY) 
{
	int rows = neurons.size();
	if (rows == 0) return {};
...
void NeuronGrowth::PrintOutNeurons(vector<vector<int>> neurons) 
{
	...
	for (int i = 0; i < neurons.size(); i+=1*dwnRatio) {
		ierr = PetscPrintf(PETSC_COMM_WORLD, "| ");
		...
		for (int i = 0; i < user->Gpt.size(); i++) {
			for (int j = 0; j < user->Gpt.size(); j++) {
				...
				if (user->n < 50) {
					...
					// if (((user->vars[9] > 0) && (user->vars[9] < 1)) || (user->vars[9] > 2)) {
					// if (user->vars[9] > 0.2) {
					if (user->vars[9] > 0) {
					...
					// std::cout << user->vars[7] << " " << user->Regular_Heiviside_fun(50 * user->vars[7] - 0) << " " << user->r * user->vars[7] - user->g << " " << user->Regular_Heiviside_fun(user->r * user->vars[7] - user->g) << std::endl;
				}

				// user->vars[8] = user->alphaOverPi*atan(user->gamma * user->Regular_Heiviside_fun(user->r * user->vars[7] - user->g) * (1 - user->vars[6]));

				// calculate C1 variable for phase field energy term
				user->vars[0] = user->vars[8] - user->pre_C0[ind]; // C1 = E - (0.5 + 6 * user->s_coeff * sqrt(pow(dThedx, 2) + pow(dThedy, 2));
...
int RunNG(int n_bzmesh, vector<vector<int>> ele_process_in, vector<Vertex2D> cpts_initial, const vector<Element2D> tmesh_initial, vector<Vertex2D> &cpts, vector<Vertex2D> prev_cpts, const vector<Element2D> &tmesh, string path_in, string path_out, int &iter, int end_iter_in,
	vector<vector<float>> &NGvars, int &NX, int &NY, vector<array<int, 2>> &seed, int &originX, int &originY, vector<int> &rfid, vector<int> &rftype, bool &localRefine)
{	
	/*========================================================*/
	// Initializations
	NeuronGrowth NG;
	...
	vector<float> geodist_lf, id, id_lf, tips;
	...
	id_lf = InterpolateVars(neurons, cpts_initial, cpts, 0);
	id = ConvertTo1DFloatVector(neurons);
	NG.DetectTips(NG.phi, id, NG.numNeuron, tips, NX, NY);
	...
	while (iter <= NG.end_iter) {
		...
		if (NG.judge_phi == 0) { 
			...
			// NG.ierr = SNESSetFunction(NG.snes_phi, NULL, FormFunction_phi_wip, &NG); CHKERRQ(NG.ierr);
		...
		// /*#################################################################*/
		// /*Explicit linear Phase field equation*/
		// NG.phi_prev = NG.phi;
		// // if (NG.judge_phi == 0)
		// MatZeroEntries(NG.GK_phi);
		// VecSet(NG.GR_phi, 0);
		// NG.BuildLinearSystemProcessNG_phi(tmesh, cpts);
		// VecAssemblyEnd(NG.GR_phi);
		// // if (NG.judge_phi == 0)
		// MatAssemblyEnd(NG.GK_phi, MAT_FINAL_ASSEMBLY);

		// /*Petsc solver setting*/
		// if (NG.judge_phi == 0) {
		// 	KSPCreate(PETSC_COMM_WORLD, &NG.ksp_phi);
		// 	KSPSetOperators(NG.ksp_phi, NG.GK_phi, NG.GK_phi);
		// 	KSPGetPC(NG.ksp_phi, &NG.pc_phi);
		// 	PCSetType(NG.pc_phi, PCBJACOBI);
		// 	KSPSetType(NG.ksp_phi, KSPGMRES);	
		// 	KSPGMRESSetRestart(NG.ksp_phi, 100);
		// 	KSPSetInitialGuessNonzero(NG.ksp_phi, PETSC_TRUE);
		// 	KSPSetTolerances(NG.ksp_phi, 1.e-8, PETSC_DEFAULT, PETSC_DEFAULT, 100000);
		// 	KSPSetFromOptions(NG.ksp_phi);
		// 	KSPSetUp(NG.ksp_phi);
		// 	if (NG.n == 0)
		// 		KSPView(NG.ksp_phi, PETSC_VIEWER_STDOUT_WORLD);
		// 	NG.judge_phi = 1;
		// }

		// KSPSolve(NG.ksp_phi, NG.GR_phi, NG.temp_phi);
		// PetscInt its_phi;
		// KSPGetIterationNumber(NG.ksp_phi, &its_phi);
		// KSPConvergedReason reason_phi;
		// KSPGetConvergedReason(NG.ksp_phi, &reason_phi);
		// if (reason_phi != 2)
		...
		// // /*========================================================*/
		// /*Implcit Non-liear NR solver for Phase field equation*/	
		// NG.phi_prev = NG.phi; // phi_prev -> phi_k
		// NG.phi_guess = NG.phi_prev; // phi_prev -> phi_k+1
		// NG.dp = NG.phi; // dp -> delta phi
		
		// int iter = 0;
		// float nrTol = 1e-4;
		// float err = 2*nrTol; // initialize error to be larger than tol
		// // PetscReal vecMax;
		// PetscInt its_phi;
		// KSPConvergedReason reason_phi;
		// while (err > nrTol) {
		...
		// 		KSPGetPC(NG.ksp_phi, &NG.pc_phi);
		// 		PCSetType(NG.pc_phi, PCBJACOBI);
		// 		// PCSetType(NG.pc_phi, PCJACOBI);
		// 		// PCSetType(NG.pc_phi, PCSOR);
		// 		// PCSetType(NG.pc_phi, PCEISENSTAT);
		// 		// PCSetType(NG.pc_phi, PCICC);
		// 		// PCSetType(NG.pc_phi, PCILU);
		// 		// PCSetType(NG.pc_phi, PCKSP);
		// 		// PCSetType(NG.pc_phi, PCMPI);
		// 		PCSetType(NG.pc_phi, PCML);
				
		// 		// PCSetType(NG.pc_phi, PCLU);
		// 		KSPSetType(NG.ksp_phi, KSPGMRES);
		// 		KSPGMRESSetRestart(NG.ksp_phi, 100);
		// 		KSPSetInitialGuessNonzero(NG.ksp_phi, PETSC_TRUE);
		// 		KSPSetTolerances(NG.ksp_phi, 1.e-8, PETSC_DEFAULT, PETSC_DEFAULT, 100000);
		...
		// 	VecScatterCreateToAll(NG.temp_phi, &ctx_phi, &temp_phi_seq);
		// 	VecScatterBegin(ctx_phi, NG.temp_phi, temp_phi_seq, INSERT_VALUES, SCATTER_FORWARD);
		// 	VecScatterEnd(ctx_phi, NG.temp_phi, temp_phi_seq, INSERT_VALUES, SCATTER_FORWARD);
		// 	VecGetArray(temp_phi_seq, &_p);
		// 	err = 0;
		// 	for (int i = 0; i < NG.phi_guess.size(); i++)
		// 	{
		// 		NG.dp[i] = PetscRealPart(_p[i]);
		// 		NG.phi_guess[i] = NG.phi_prev[i] + NG.dp[i];
		// 		if (abs(NG.dp[i]) > err)
		// 			err = abs(NG.dp[i]);
		// 	}
		// 	VecRestoreArray(temp_phi_seq, &_p);
		// 	VecScatterDestroy(&ctx_phi);
		// 	VecDestroy(&temp_phi_seq);

		// 	NG.phi_prev = NG.phi_guess;
		// 	iter += 1;
		// 	PetscPrintf(PETSC_COMM_WORLD, "NR iter: %d | NR err: %.9f | KSPreason: %d | KSPiter: %d\n", iter, err, reason_phi, its_phi);		
		// 	if (iter > 20) {
		// 		PetscPrintf(PETSC_COMM_WORLD, "Unlikely to converge after %d iterations\n", iter);	
		// 		exit(0);
		// 	}
		// 	MPI_Barrier(PETSC_COMM_WORLD);
		// }
		...
		PetscPrintf(PETSC_COMM_WORLD, "Step:%d/%d | Phi:%d[%d]%.3fs | Syn:%d[%d]%.3fs | Tub:%d[%d]%.3fs | Mesh:%dx%d |\n",\
		...
		if ((NG.n % 25 == 0) && (NG.n != 0)) {	
			...
			// vector<float> neuronsF;	neuronsF.clear(); neuronsF.resize(id.size());
			NG.IdentifyNeurons(neurons, seed, NX, NY, originX, originY);
			vector<float> id = ConvertTo1DFloatVector(neurons);

			vector<float> neuronsF = ConvertTo1DFloatVector(neurons);
			NG.DetectTips(neuronsF, id, NG.numNeuron, tips, NX, NY);
			...
			for (int i = 0; i < geodist.size(); i++) {
				...
				if (localMaximaMatrix[i] == 0) {
					Mphi[i] = 0;
				} else {
					Mphi[i] = 10;
				}
			}
			// if (NG.n > 500) { // increase Mphi at longest tip 
			// 	for (int i = -1; i < 1; i++) {
			// 		for (int j = -1; j < 1; j++) {
		...
		if ((NG.n % 100 == 0) && (NG.n != 0)) {
			...
			if (expd_dir <= 3) {
				switch (expd_dir) {
					case 0: // left
						NX += 6;	originX -= 3;		break;
					case 1: // top
						NY += 6;				break;
					case 2: // right
						NX += 6;				break;
					case 3: // bottom
						NY += 6;	originY -= 3;		break;
				}

				// // Domain expansion without local refinement
				// NGvars.clear(); NGvars.resize(6);
				// NG.ExpandDomain(NG.phi, NGvars[0], expd_dir, NX, NY);
				// NG.ExpandDomain(NG.syn, NGvars[1], expd_dir, NX, NY);
				// NG.ExpandDomain(NG.tub, NGvars[2], expd_dir, NX, NY);
				// NG.ExpandDomain(NG.theta, NGvars[3], expd_dir, NX, NY);
				// NG.PopulateRandom(NGvars[3]); // replace 0 with random number (for theta)
				// NG.ExpandDomain(NG.phi_0, NGvars[4], expd_dir, NX, NY);
				// NG.ExpandDomain(NG.tub_0, NGvars[5], expd_dir, NX, NY);
