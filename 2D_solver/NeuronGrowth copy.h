...
public:
	...
	int var_save_invl,png_save_invl,png_plot_invl,numNeuron,gc_sz,M_axon,M_neurites,end_iter,aniso,gamma,seed_radius;
	float kappa,dt,Dc,alpha,alphaOverPi,tau,M_phi,M_theta,s_coeff,delta,epsilonb,r,g,alphaT,betaT,Diff,source_coeff;

	// // Parameters for neuron growth model
	// int tip_invl,expd_invl,gc_sz,aniso,gamma,seed_radius,numNeuron,end_iter;
	// float kappa,dt,Dc,alpha,alphaOverPi,M_phi,M_tip,M_axon,s_coeff,delta,epsilonb,r,g,alphaT,betaT,Diff,source_coeff;

	// int var_save_invl,png_save_invl,png_plot_invl;

	// Initializations
	NeuronGrowth();
	void AssignProcessor(vector<vector<int>> &ele_proc); // assign elements to different processors
	void InitializeProblemNG(const int n_bz, vector<Vertex2D>& cpts, vector<Vertex2D> prev_cpts, vector<vector<float>> &NGvars, vector<array<int, 2>> &seed);
	...
	// void BuildLinearSystemProcessNG_phiNR(const vector<Element2D> &tmesh, const vector<Vertex2D> &cpts);
	// void BuildLinearSystemProcessNG_phi(const vector<Element2D> &tmesh, const vector<Vertex2D> &cpts);
	...
	// void DetectTips(vector<float> phi, vector<float> &tips); // for square domain
	// void DetectTips(vector<float> phi, vector<float> &tips, int NX, int NY); // for when NX != NY
	void DetectTips(vector<float> phi_in, vector<float> id, int numNeuron, vector<float> &tips, int NX, int NY); // for multiple neurons
	...
	void IdentifyNeurons(vector<vector<int>>& neurons, vector<array<int, 2>> seed, int NX, int NY, int originX, int originY); // to detect neurons
	...
	vector<vector<int>> CalculateGeodesicDistanceFromPoint(vector<vector<int>> neurons, vector<array<int, 2>> &seed, int originX, int originY); // multiple neurons
