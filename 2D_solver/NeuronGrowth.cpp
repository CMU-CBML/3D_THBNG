...
{	
	...
	while (iter <= NG.end_iter) {
		...
		if ((NG.n % NG.var_save_invl == 0) && (NG.n != 0)) {	
			...
			vector<float> neuronsF = ConvertTo1DFloatVector(neurons); // need to be changed
			...
			// varName = "Mphi_lf_running_";
			// NG.VisualizeVTK_ControlMesh(cpts, tmesh, NG.n, path_out, NG.Mphi, varName); // solution on control points
