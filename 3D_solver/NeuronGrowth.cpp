...
NeuronGrowth::NeuronGrowth(){
	...
	// end_iter = 5000;     		// end iteration
	aniso 			= 6;   		// aniso
	gamma 			= 10;  		// gamma
	// seed_radius 		= 20;		// seed radius
	...
	Diff			= 60;		// Diff
	source_coeff		= 15;		// source_coeff
	// source_coeff		= 2e-5;		// source_coeff
}
...
void NeuronGrowth::InitializeProblemNG(const int n_bz, vector<Vertex3D>& cpts, vector<Vertex3D> prev_cpts, vector<vector<float>> &NGvars, vector<array<int, 3>> &seed)
{
	...
	for (int i = 0; i<cpts.size(); i++) {
		if (i==0) {
			...
			max_z = max(cpts[i].coor[1],max_z);
			min_z = min(cpts[i].coor[1],min_z);
		}
	}
	...
	if (n == 0) { // Initialize
		...
		for (int i = 0; i<cpts.size(); i++) {
			...
			for (int j = 0; j<seed.size(); j++) { // multiple neurons
				r = sqrt((x-(float)seed[j][0])*(x-(float)seed[j][0])+(y-(float)seed[j][1])*(y-(float)seed[j][1])+(z-(float)seed[j][2])*(z-(float)seed[j][2])); // radius of initial soma
	...
	ierr = MatMPIAIJSetPreallocation(J, 125, NULL, 125, NULL);
	...
	ierr = MatMPIAIJSetPreallocation(GK_syn, 125, NULL, 125, NULL);
	...
	ierr = MatMPIAIJSetPreallocation(GK_tub, 125, NULL, 125, NULL);
...
void NeuronGrowth::ConcentrationCal_Coupling_Bezier(double u, double v, double w, const Element3D& bzel, double pt[3], double& disp, double dudx[3], double& detJ)
{
	double dUdx[3][3];
...
void NeuronGrowth::WriteVTK(const vector<array<double, 3>> spt, const vector<double> sdisp, const vector<array<int, 8>> sele, int step, string fn)
{
	stringstream ss;
	ss << step;
...
void NeuronGrowth::WriteVTK(const vector<array<double, 3>> spt, const vector<vector<float>> sdisp, const vector<array<int, 8>> sele, int step, string fn)
{
	stringstream ss;
	ss << step;
	string fname = fn + "/physics_allparticle_" + ss.str() + ".vtk";
...
void NeuronGrowth::ElementEvaluationAll_syn_tub(const int nen, const vector<float> &Nx, vector<array<float, 3>> &dNdx, const vector<float> elePhiDiff, float &elePf, const vector<float> eleSyn, float &eleS, const vector<float> elePhi, float &eleP, const vector<float> elePhiPrev, float &elePprev, const vector<float> eleConct, float &eleC, float &dPdx, float &dPdy, float &dPdz)
{
	...
	for (int i = 0; i < nen; i++) {
		...
		dPdx += elePhi[i] * dNdx[i][0];	dPdy += elePhi[i] * dNdx[i][1];	dPdz += elePhi[i] * dNdx[i][1];
	}
}
...
void NeuronGrowth::ElementEvaluationAll_syn_tub(const int nen, const vector<float> &Nx, vector<array<float, 3>> &dNdx, vector<vector<float>> &eleVal, vector<float> &vars)
{
	...
	for (int i = 0; i < nen; i++) {
		...
		vars[3] += eleVal[2][i] * dNdx[i][0];	vars[4] += eleVal[2][i] * dNdx[i][1]; 	vars[11] += eleVal[12][i] * dNdx[i][2];
	}
...
void NeuronGrowth::EvaluateOrientation(const int nen, const vector<float> &Nx, const vector<array<float, 3>> &dNdx, const vector<float> elePhi, const vector<float> eleTheta,  vector<float>& eleEpsilon, vector<float>& eleEpsilonP)
{
	for (int i = 0; i < nen; i++) {
		eleEpsilon[i] += epsilonb * (1.0 + delta * cos(aniso * (atan2(elePhi[i] * dNdx[i][0], elePhi[i] * dNdx[i][1], elePhi[i] * dNdx[i][2]) - eleTheta[i] * Nx[i])));
		eleEpsilonP[i] += -epsilonb * (aniso * delta * sin(aniso * (atan2(elePhi[i] * dNdx[i][0], elePhi[i] * dNdx[i][1], elePhi[i] * dNdx[i][2]) - eleTheta[i] * Nx[i])));
	}
}

void NeuronGrowth::BuildLinearSystemProcessNG_syn(const vector<Element3D> &tmesh, const vector<Vertex3D> &cpts)
{
	/*Build linear system in each process*/
	for (int e = 0; e < bzmesh_process.size(); e++) {
		float detJ;
...
void NeuronGrowth::CalculateSumGradPhi0(const vector<Element3D> &tmesh, const vector<Vertex3D> &cpts)
{
	/*Build linear system in each process*/
	for (int e = 0; e < bzmesh_process.size(); e++) {
		float detJ;
...
void NeuronGrowth::BuildLinearSystemProcessNG_tub(const vector<Element3D> &tmesh, const vector<Vertex3D> &cpts)
{
	/*Build linear system in each process*/
	for (int e = 0; e < bzmesh_process.size(); e++) {
		float detJ;
...
void NeuronGrowth::BuildLinearSystemProcessNG_syn_tub(const vector<Element3D> &tmesh, const vector<Vertex3D> &cpts)
{
	/*Build linear system in each process*/
	int ind(0);
	for (int e = 0; e < bzmesh_process.size(); e++) {
...
int NeuronGrowth::CheckExpansion3D(vector<float> input, int NX, int NY, int NZ) 
{
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 1; j < NY; j++) {
			// Check left and right directions in the XY plane
		...
		for (int j = 1; j < NX; j++) {
			// Check top and bottom directions in the XZ plane
		...
		for (int j = 1; j < NZ; j++) {
			// Check front and back directions in the YZ plane
			if (round(input[i * (NY + 1) + j * (NY + 1) * (NZ + 1)]) > 0) {
				return 4;
			} else if (round(input[(NX + 1 - i - 1) * (NY + 1) + j * (NY + 1) * (NZ + 1)]) > 0) {
				return 5;
			}
		}
	}
...
void NeuronGrowth::DetectTipsMulti3D(vector<float> id, int numNeuron, vector<float> &tips, int NX, int NY, int NZ)
{
	...
	for (int i = (5 * (NY + 1) + 5); i < (length - 4 * (NY + 1) - 4); i++) {
		tips[i] = 0;
	...
	for (int i = 1 + (NY + 1); i < length - (NY + 1) - 1; i++) {
		if (tips[i] < (threshold * maxVal)) {
			tips[i] = 0;
		} else {
			tips[i] = 1;
...
{
	neurons = ConvertTo3DIntVector(phi, NX + 1, NY + 1, NZ + 1);  // Assuming ConvertTo3DIntVector is defined
...
vector<vector<array<int, 3>>> NeuronGrowth::NeuriteTracing(vector<vector<double>> distance) 
{
...
{	
	...
	NG.IdentifyNeurons3D(neurons, seed, NX, NY, originX, originY);
	distances = NG.CalculateGeodesicDistanceFromPoint3D(neurons, seed, originX, originY);
	PetscPrintf(PETSC_COMM_WORLD, "Calculated geodesic distances!-----------------------------------------------\n");
	id = ConvertTo1DFloatVector(neurons);
	NG.DetectTipsMulti3D(id, NG.numNeuron, tips, NX, NY);
	...
	while (iter <= NG.end_iter) {
		...
		if ((NG.n % 5 == 0) && (NG.n != 0)) {	
			...
			NG.DetectTipsMulti3D(id, NG.numNeuron, NG.tips, NX, NY);
