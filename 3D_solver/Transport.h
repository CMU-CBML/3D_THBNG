...
private:
	...
	void BuildLinearSystemProcessNG(const vector<Element3D>& tmesh, const vector<Vertex3D> &cpts, const vector<array<double, 3>> velocity_node, const double Vplus, const double Vminus);
...
public:	
	...
	void RunNG(const vector<Vertex3D>& cpts, const vector<array<double, 3>> velocity_node, const vector<Element3D> &tmesh, string path_in, string path_out);
