...
void Transport::RunNG(const vector<Vertex3D>& cpts, vector<int> var_i, vector<double> var_d, const vector<Element3D> &tmesh, string path_in, string path_out)
{
	PetscPrintf(PETSC_COMM_WORLD, "Running Neuron Growth Simulation...\n");
	ReadBezierElementProcess(path_in);
	...
	VecSet(GR, 0);
	t0 = time(NULL);
	BuildLinearSystemProcessNG(tmesh, cpts, velocity_node, par[1], par[2]);
	VecAssemblyEnd(GR);
	PetscPrintf(PETSC_COMM_WORLD, "Done Vector Assembly...\n");
	if (judge == 0)
		MatAssemblyEnd(GK, MAT_FINAL_ASSEMBLY);
...
void Transport::InitializeProblemNG(const int n_bz, vector<int> var_i, vector<double> var_d, int cpt_sz, const vector<double>& var);
{
	int i, j, e;
...
void Transport::BuildLinearSystemProcessNG(const vector<Element3D>& tmesh, const vector<Vertex3D> &cpts, vector<int> var_i, vector<double> var_d)
{
	/*Build linear system in each process*/
	int e;
	cout << "Process:" << comRank << " out of " << nProcess << " Start Loop for " << bzmesh_process.size() << " elements.\n";
